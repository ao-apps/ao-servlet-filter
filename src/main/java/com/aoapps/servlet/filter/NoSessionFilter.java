 ...
 * Copyright (C) 2010, 2011, 2013, 2016, 2019, 2020, 2021, 2022, 2024, 2025, 2026  AO Industries, Inc.
...
public class NoSessionFilter implements Filter {
  ...
  public void init(FilterConfig config) {
    ...
    if (cookieNamesInitParam != null) {
      cookieNames.addAll(Strings.splitCommaSpace(cookieNamesInitParam));
    }
    if (cookieNames.size() > MAXIMUM_COOKIES) {
      throw new IllegalArgumentException("cookieNames.size() > " + MAXIMUM_COOKIES);
    }
  }
                ...
                public Cookie[] getCookies() {
                  ...
                  if (headerCookies != null) {
                    for (Cookie cookie : headerCookies) {
                      ...
                      if (
                          // Only add expected cookie names
                          cookieNames.contains(cookieName)
                      ) {
